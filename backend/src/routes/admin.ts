...
export function setAdminSocketIO(io: IOServer) {
  ioRef = io;
}
 ...
 */
router.get("/pending-users", authenticateToken as any, requireAdmin as any, async (req, res) => {
  try {
    ...
    logger?.error("Fetch pending users error:", error);
 ...
 */
router.post("/approve-user", authenticateToken as any, requireAdmin as any, async (req, res) => {
  try {
 ...
 */
router.post("/reject-user", authenticateToken as any, requireAdmin as any, async (req, res) => {
  try {
    const { userId } = req.body;
