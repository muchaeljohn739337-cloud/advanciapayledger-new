...
import { smsPoolService } from "../services/smsPoolService";
...
// SMS Pool service (replaces Twilio)
// SMS sending is handled by smsPoolService
...
router.post("/login", async (req, res) => {
  ...
  // Send SMS OTP via SMS Pool
  if (phone) {
    try {
      const smsResult = await smsPoolService.sendSMS(
        phone,
        `Your Advancia admin login code is ${code}. Valid for 5 minutes.`
      );

      if (smsResult.success) {
        await logAdminLogin(req, email, "OTP_SENT", phone);
        console.log(`📱 Admin OTP sent via SMS Pool to ${phone}: ${code}`);
        ...
        console.warn(`⚠️ SMS Pool failed: ${smsResult.error}`);
      }
    } catch (smsError) {
