...
export async function initializeIPTables(): Promise<void> {
  try {
    ...
    await prisma.$executeRawUnsafe(`DROP TABLE IF EXISTS ip_whitelist CASCADE;`);
    await prisma.$executeRawUnsafe(`DROP TABLE IF EXISTS ip_blacklist CASCADE;`);

    // Create tables with correct schema (added_by as TEXT to match users.id)
