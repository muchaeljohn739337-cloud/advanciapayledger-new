...
import { getAvailableImageSizes, ImageGenerationRequest } from '@/services/aiGenerator';
import { AlertCircle, Download, ImageIcon, Loader2, Send, Sparkles } from 'lucide-react';
...
export default function ImageGeneratorTab() {
  ...
  const { loading, error, result, progress, generateImageContent, resetState } = useAIGenerator();
  ...
  const handleGenerate = async () => {
    ...
    try {
      await generateImageContent({
        prompt,
        size,
        captchaToken: '', // TODO: Implement CAPTCHA widget
      });
    } catch (err) {
    ...
    <div className="p-6">
      ...
      <div className="mt-4 p-3 bg-blue-900/20 border border-blue-700/50 rounded-lg">
        <p className="text-sm text-blue-200">
          💡 <strong>Tip:</strong> Be specific and descriptive. Include details about style, mood,
          colors, and composition for best results.
        </p>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-200">
              <p className="font-medium">
                ...
                {progress.status === 'completed' && 'Image created successfully!'}
                {progress.message}
              </p>
            </div>
          </div>
        ...
        <div className="mt-6">
          ...
          <div className="mt-3 p-3 bg-yellow-900/20 border border-yellow-700/50 rounded-lg">
            <p className="text-sm text-yellow-200">
              ⚠️ <strong>Note:</strong> Image URLs from DALL-E are temporary. Download your image to
              save it permanently.
            </p>
          </div>
        </div>
