...
export async function POST(request: NextRequest) {
  try {
    ...
    const assessment: LoginRiskAssessment = {
      ...
      action: 'ALLOW',
      reason: 'Login attempt appears legitimate',
    };
    ...
    if (Math.random() < 0.1) {
      ...
      assessment.factors.deviceTrust = 40;
      assessment.action = 'CHALLENGE';
      assessment.reason = 'Unrecognized device detected - Require additional verification';
    }

    return NextResponse.json(assessment);
  } catch (error) {
    console.error('Login assessment error:', error);
    return NextResponse.json({ error: 'Failed to assess login risk' }, { status: 500 });
  }
}
