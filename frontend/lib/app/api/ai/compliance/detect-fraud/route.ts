...
function analyzeTransactionRisk(data: TransactionData): {
  ...
  const maxPossibleScore = riskFactors.reduce((sum, f) => sum + 100 * f.weight, 0);
...
function analyzeGeographicRisk(
  ...
  } else if (location?.country && moderateRiskCountries.includes(location.country)) {
    score = 45;
    severity = 'medium';
...
function analyzeMerchantRisk(category?: string): RiskFactor {
  ...
  } else if (category && moderateRiskCategories.includes(category.toLowerCase())) {
    score = 35;
    severity = 'medium';
...
function determineAction(normalizedScore: number): 'MONITOR' | 'FLAG' | 'BLOCK' | 'INVESTIGATE' {
  if (normalizedScore >= 80) return 'BLOCK';
  if (normalizedScore >= 60) return 'INVESTIGATE';
  if (normalizedScore >= 40) return 'FLAG';
  return 'MONITOR';
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { riskFactors, totalScore, maxPossibleScore } = analyzeTransactionRisk(body);
    ...
    const patterns = riskFactors.filter((f) => f.score > 30).map((f) => f.description);
